...
public class ChatImageBehavior extends CoordinatorLayout.Behavior<FrameLayout> {
    ...
    public ChatImageBehavior(Context context, View finalView, AttributeSet attrs) {
        ...
        //计算出头像居右的距离
        marginRight = UITool.dipToPx(context, 35);
...
//        Log.d(TAG, "layoutDependsOn");
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, FrameLayout child, View dependency) {
        ...
        //计算出头像的起始x坐标
        if (startX == 0)
            startX = (int) (dependency.getWidth() / 2 + UITool.dipToPx(parent.getContext(), 20));
        //计算出头像的起始y坐标
        if (startY == 0)
        ...
        float moveX = expandedPercentageFactor * (dependency.getWidth() - startX - marginRight - 2 * minChildWidth);
        //更新头像的位置
