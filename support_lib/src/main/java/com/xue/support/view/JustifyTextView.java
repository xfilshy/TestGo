...
public class JustifyTextView extends AppCompatTextView {
    ...
    public JustifyTextView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.JustifyTextView_lineSpacing) {
                lineSpacing = a.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 3, getResources().getDisplayMetrics()));
            }
    ...
    private void arrayToWords() {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (array[i] >= 0 && array[i] < 0x7f) {
                ...
                if (array.length - 1 > i + 1 && (array[i + 1] == ' ' || array[i + 1] == ' ')) {
                    j++;
                }
